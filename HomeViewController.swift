...
class HomeViewController: UIViewController, CLLocationManagerDelegate, locationDelegateToHome, filterDelegate, delegate, UISearchBarDelegate {
    ...
    var myUserId : String = String()
    var isFromHomeViewLoad : Bool = Bool()
    var isAll : Bool = Bool()
    var isFromUpdateLocation : Bool = Bool()
    
    
    //MARK:- ViewLifeCycle
    override func viewDidLoad() {
        super.viewDidLoad()
        self.tblviewHome.register(FeedCell.self)
        self.setNavigationBar(image : searchImage , title: "Feed", target: self, action: #selector(btnSerch))
        self.setRightButtonOnNavigationBar(image : notificationImage , target: self, action: #selector(notificationListBtnClick))
        SingletonClass.sharedInstance().isComeToHome = true
        allLocation = "All"
        lblLocation.text = allLocation
        isAll = true
        allLocation = ""
        isFromHomeViewLoad = true
        isFromUpdateLocation = false
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if SingletonClass.sharedInstance().isComeToHome == true{
            setUI()
        }
    ...
    func setUI(){
        ...
        lblForNotificationCount.layer.masksToBounds = true
        pageNo = 0
        homeTaskAPI()
    ...
    func redirectOnNotification() {
        if UserDefaults.standard.value(forKey: "IsCheckSelectedSegmentIndex") != nil
        {
            ...
            if selectedIndex == "5"{
                let vc = Router.sharedInst().getMyProfileVC()
                self.navigationController?.pushViewController(vc, animated: true)
            }
            else{
                let vc = Router.sharedInst().getMyTaskParentVC()
                self.navigationController?.pushViewController(vc, animated: true)
            }
        }
    }
    ...
    @objc func btnAcceptTask(sender : UIButton){
        ...
        if myUserId == arrHomeTask[row].user_id{
            CommanUtil.showAlert(withTitle: AdQubes, withMsg: "You can't accept your own project", in: self)
        }else{
    ...
    @objc func btnSkipAction(sender : UIButton){
        ...
        let row = sender.tag
        if myUserId == arrHomeTask[row].user_id{
            CommanUtil.showAlert(withTitle: AdQubes, withMsg: "You can't skip your own project", in: self)
        }else{
            taskId = arrHomeTask[row].task_id
    ...
    func dataPassBoolToHome(data : Bool){
        ...
        long = ""
        allLocation = "All"
        dicDataComeFromFilter.removeAll()
    ...
    {
        let topRatedUserVC = Router.sharedInst().getTopRatedUserVC()
        self.navigationController?.pushViewController(topRatedUserVC, animated: true)
    }
    
    @IBAction func btnFilter(_ sender: Any) {
        
        let controller = Router.sharedInst().getFilterVC()
    ...
    @objc func btnUserProfile(sender : UIButton){
        ...
        let controller = Router.sharedInst().getProfileVC()
    ...
    @objc func btnOption(sender : UIButton){
            ...
            ServicesClass.postDataFromURL(url: APIConstants.TaskReportByUser, parameters: param, requestName: "") { (json, error) in
                ...
                if error == nil{
                    if json!["status"] as? Int != nil && json!["status"] as? Int == 1{
                        ...
                        MBProgressHUD.hide(for: self.view, animated: true)
                        CommanUtil.showAlert(withTitle: AdQubes, withMsg: strMsg, in: self)
                    }else{
                        let strMsg = json!["message"] as! String
                        MBProgressHUD.hide(for: self.view, animated: true)
                        CommanUtil.showAlert(withTitle: AdQubes, withMsg: strMsg, in: self)
                    ...
                    MBProgressHUD.hide(for: self.view, animated: true)
                    CommanUtil.showAlert(withTitle: AdQubes, withMsg: (error?.localizedDescription)!, in: self)
                }
            }
        })
        ...
        let action2 = UIAlertAction(title: "Hide this Post", style: .default, handler: { (action) -> Void in
            let row = sender.tag
            self.taskId = self.arrHomeTask[row].task_id
            self.skipAPI(taskId : self.taskId)
            self.arrHomeTask.remove(at: row)
            let cell = self.tblviewHome.cellForRow(at: IndexPath.init(row: row, section: 0))
            let indexPath = self.tblviewHome.indexPath(for: cell!)
            self.tblviewHome.deleteRows(at: [indexPath!], with: .left)
        ...
        self.geoCoder.reverseGeocodeLocation(location, completionHandler: { (placemarks, error) -> Void in
            ...
            if placemarks != nil {
                ...
                if let state = placeMark.administrativeArea{
                    self.arrLocationDetail.append(state)
                    print(state)
                }
    ...
    func acceptTaskAPI(taskId : String , task_end_date : String){
        ...
        param["task_end_date"] = task_end_date//AppHelper.convertDateFormater(task_end_date)
        ...
        ServicesClass.postDataFromURL(url: APIConstants.accept_task, parameters: param, requestName: "") { (json, error) in
            ...
            if error == nil{
                ...
                if json!["status"] as? Int != nil && json!["status"] as? Int == 1{
                    ...
                    MBProgressHUD.hide(for: self.view, animated: true)
                    CommanUtil.showAlert(withTitle: AdQubes, withMsg: json!["message"] as! String, in: self)
                }
    ...
    func homeTaskAPI() {
        ...
        if strForLocationVal.isEmpty{
            ...
            isFromHomeViewLoad = false
        }
        else{
            if isCheckSelectAllBtnClick == true || isFromHomeViewLoad == true{
                param["address"] = allLocation //""
        ...
        if isFromResetButton {
            ...
            if !dicDataComeFromFilter.isEmpty {
                ...
                param["max_kube"] = "10000"
                param["min_distance"] = "0" //dicDataComeFromFilter["lowerValueDistance"] as? String
        ...
        ServicesClass.postDataFromURL(url: APIConstants.hometask, parameters: param, requestName: "") { (json, error) in
            self.isFromUpdateLocation = false
            MBProgressHUD.hide(for: self.view, animated: true)
            self.tblviewHome.infiniteScrollingView.stopAnimating()
                ...
                if json!["status"] as? Int != nil && json!["status"] as? Int == 1{
                    ...
                    if let homeTask : Array<Dictionary<String,Any>> = json!["hometask"] as? Array<Dictionary<String,Any>> {
                        ...
                        if homeTask.count > 0{
                            ...
                            if self.isPullToRefresh || self.pageNo == 0{
                            ...
                            for dic in homeTask{
                                ...
                                let model : HomeModel = HomeModel.init(task_id: "", task_title: "", task_description: "", task_credit: "", task_end_date: "", created_at: "", task_visibility: "", user_id: "", full_name: "", profile_pic_image: "", location_address: "", category_name: "", location_country: "", location_city: "", location_state: "")
                                ...
                                if let location_country : String = dic["location_country"] as? String{
                                    model.location_country = location_country
                                }
                                
                                if let location_city : String = dic["location_city"] as? String{
                                    model.location_city = location_city
                                }
                                
                                if let location_state : String = dic["location_state"] as? String{
                                    model.location_state = location_state
                                }
                            ...
                            if self.pageNo == 0 {
                                ...
                                self.noDataLabel.font = UIFont(name: "Avenir-Light", size: 15)
                                self.noDataLabel.text = "Sorry, there are no projects yet\n for this location" //
                                self.noDataLabel.textColor = placeHolderColor
...
extension HomeViewController : UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if myUserId == arrHomeTask[indexPath.row].user_id{
            cell.btnOptions.isHidden = true
        }else{
            cell.btnOptions.isHidden = false
        }
        
        let str = arrHomeTask[indexPath.row].task_description
        let nsString = str! as NSString
        if nsString.length >= 150
        {
            cell.lblTaskDescription.text = nsString.substring(with: NSRange(location: 0, length: nsString.length > 150 ? 150 : nsString.length)) + "....."
        }else{
        ...
        cell.lblTitle.text = arrHomeTask[indexPath.row].task_title
        if arrHomeTask[indexPath.row].location_country == ""{
            if arrHomeTask[indexPath.row].task_visibility == "2"{ // 1 for location, 2 for everyone, 3 for performer
                cell.lblLocation.text = "Location : Universe"
            }
            if arrHomeTask[indexPath.row].task_visibility == "3"{
                cell.lblLocation.text = "Location : N/A(task assigned to performer)"
            }
            
        }else{
            let city : String = arrHomeTask[indexPath.row].location_city
            let state : String = arrHomeTask[indexPath.row].location_state
            
            if city == ""{
                if state != ""{
                    cell.lblLocation.text = "Location : \(arrHomeTask[indexPath.row].location_state!), \(arrHomeTask[indexPath.row].location_country!)"
                }else{
                    cell.lblLocation.text = "Location : \(arrHomeTask[indexPath.row].location_country!)"
                }
            }else{
                if state != ""{
                    cell.lblLocation.text = "Location : \(arrHomeTask[indexPath.row].location_city!), \(arrHomeTask[indexPath.row].location_state!), \(arrHomeTask[indexPath.row].location_country!)"
                }else{
                    cell.lblLocation.text = "Location : \(arrHomeTask[indexPath.row].location_city!), \(arrHomeTask[indexPath.row].location_country!)"
                }
            }
        }
        ...
        cell.btnSkip.layer.borderWidth = 1
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let controller = Router.sharedInst().getTaskDetailVC()
        controller.delegate = self
        ...
        controller.dicReceiveTaskIdAndTaskEndDateFromInitiatePending["other_userId"] = arrHomeTask[indexPath.row].user_id
        self.navigationController?.pushViewController(controller, animated: true)
